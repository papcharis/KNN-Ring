...
double distRowMajor(double const * const X, double const * const Y,
  ...
  /* compute distance */
  //i INDEX
  // j apo 0 mexri n gia ola ta QUERY
  //
  double dist = 0;
  for (int l = 0; l < d; l++){
  ...
  for (int j = 0; j < m; j++ ){
    ...
    for (int l = 0; l < k; l++ ){
      ...
      if (ap == COLMAJOR){
        ...
        if ( fabs( knnresndist_cm(j,l) - distxy ) / (distxy + DBL_EPSILON) > 1e-6 ) {

          return 0;
        }
        /* distances should be non-decreasing */
        if ( knnresndist_cm(j,l) < maxDist ){
        ...
        /* make sure reported distance is correct */
        if ( fabs( knnresndist_rm(j,l) - distxy ) / (distxy + DBL_EPSILON) > 1e-6 ){
        //  printf("PERIMENW :%lf \n" , distxy);
        //  printf("PAIRNW :%lf \n" , knnresndist_rm(j,l));
